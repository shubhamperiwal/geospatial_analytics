                             ...
                             mainPanel(
                               fluidRow(
                                 ...
                                 column(6,
                                        conditionalPanel(
                                          condition = "input.test !='all'",
                                          h3("Box Plot for Distribution of Mean Min Distance for Region/Planning Area"),
                               ...
                               fluidRow(
                                 h4("*Criteria must be consistent to plot AHP plot")
                               ),
                               fluidRow(
                                 column(12,
                                        conditionalPanel(
                                          condition = "input.test !='all'",
                                          h3("Boxplot for Distribution of Mean Min Distance for Subzones"),
...
server <- function(input, output, session) {
  ...
  resetAHP <- function(){
    ...
    updateNumericInput(session, inputId = 'bs_clinic', value = 9)
    updateNumericInput(session, inputId = 'bs_hawker', value = 7)
    updateNumericInput(session, inputId = 'bs_mrt', value = 5)
    updateNumericInput(session, inputId = 'bs_school', value = 3)
    updateNumericInput(session, inputId = 'bs_spf', value = 1)
    updateNumericInput(session, inputId = 'clinic_hawker', value = -3)
    updateNumericInput(session, inputId = 'clinic_mrt', value = -5)
    updateNumericInput(session, inputId = 'clinic_school', value = -7)
    updateNumericInput(session, inputId = 'clinic_spf', value = -9)
    updateNumericInput(session, inputId = 'hawker_mrt', value = -3)
    updateNumericInput(session, inputId = 'hawker_school', value = -5)
  ...
  output$cmap <- renderLeaflet({
    ...
    mydata <-tm_basemap(server = "OpenStreetMap", group = "Street", alpha = 1) +
      ...
      tm_fill("count", style = "jenks", palette = 'GnBu', 
              title = "Count of Facility in each Subzone") +
  ...
  ###Synchronise Data Plot and Chrolopleth Map (ADD ON)
  observe({
