    ...
    tabPanel("Project Info",
      ...
      withTags(
        div(class = "Future_work",
            h3("Future Work"),
            style = "width: 50%;",
            p("Our project can be expanded through the following:"),
            tags$ol(
    ...
    tabPanel("Sandbox",
             sidebarLayout(
               sidebarPanel(  
                 fluidRow(
                   column(10,
                          radioButtons("databutton", h3("Display map by:"),
...
server <- function(input, output) {
    ...
    if(input$test == "all"){
      ...
      if(input$region == "WHOLE SINGAPORE" ){
        ...
        if(input$type == "Region"){
          ...
          mydata <- tm_shape(mpr[mpr$Name==input$region, ]) + tm_polygons() + tm_shape(houses_sf[houses_sf$REGION_N==input$region, ]) +
            tm_dots(col='ahp',style='quantile',size=0.01) +
            tm_style("classic", bg.color="white") + tm_view(alpha = 1, basemaps = "Stamen.Watercolor") 
      ...
      if(input$region == "WHOLE SINGAPORE" ){
        ...
        if(input$type == "Subzone"){
          if(is.null(input$userinput)){
            ...
            mydata<-  tm_shape(mpsz[mpsz$REGION_N==input$region, ]) + tm_borders(lty = "dashed",col = '#d35400',lwd = 1)+
              tm_shape(mpr[mpr$Name==input$region,]) + tm_polygons(col = '#fdcb6e', alpha = 0.3, border.col = '#2f3542', lwd = 3)+
              tm_shape(houses_sf[houses_sf$REGION_N==input$region, ]) + tm_dots(col=paste(unlist(facility_dist_vector[input$test]), collapse=''), style='fixed', breaks = c(0, 100 ,200, 300, 400, 500, 5353.2), size=0.05) +
              tm_shape(clinics_sf[clinics_sf$REGION_N==input$region, ]) + tm_dots(size = .1, alpha = .5)#, shape = clinicicon) 
          }else{
            fac_sf <- facility_sf_vector[input$test][[1]]

            mydata <- tm_shape(mpsz[mpsz$SUBZONE_N==input$userinput, ]) + tm_borders(lty = "dashed",col = '#d35400',lwd = 1) +
              tm_shape(mpsz[mpsz$SUBZONE_N==input$userinput,]) + tm_polygons(col = '#fdcb6e', alpha = 0.3, border.col = '#2f3542', lwd = 3) +
              tm_shape(houses_sf[houses_sf$SUBZONE_N==input$userinput, ]) + tm_dots(col=paste(unlist(facility_dist_vector[input$test]), collapse=''), style='fixed', breaks = c(0, 100 ,200, 300, 400, 500, 5353.2), size=0.05) +
              tm_shape(fac_sf[fac_sf$SUBZONE_N==input$userinput, ])  + tm_dots(size = .3, alpha = .5) #,shape = clinicicon)
          ...
          if(is.null(input$userinput)){
            ...
            fac_sf <- facility_sf_vector[input$test][[1]]
            mydata <- tm_shape(mpsz[mpsz$PLN_AREA_N==input$userinput, ]) + tm_borders(lty = "dashed",col = '#d35400',lwd = 1) +
              tm_shape(mpa[mpa$Name==input$userinput,]) + tm_polygons(col = '#fdcb6e', alpha = 0.3, border.col = '#2f3542', lwd = 3) +
              tm_shape(houses_sf[houses_sf$PLN_AREA_N==input$userinput, ]) + tm_dots(col=paste(unlist(facility_dist_vector[input$test]), collapse=''), style='fixed', breaks = c(0, 100 ,200, 300, 400, 500, 5353.2), size=0.05) +
              tm_shape(fac_sf[fac_sf$PLN_AREA_N==input$userinput, ])  + tm_dots(size = .3, alpha = .5) #,shape = clinicicon)
          }
        }else{
          fac_sf <- facility_sf_vector[input$test][[1]]
          mydata<-  tm_shape(mpsz[mpsz$REGION_N==input$region, ]) + tm_borders(lty = "dashed",col = '#d35400',lwd = 1)+
  ...
  output$cmap <- renderLeaflet({
    if(input$radio == "busStop"){
      ...
      mydata <-tm_shape(mpsz) +tm_fill("count",style = "quantile",palette = "Blues",thres.poly = 0) + 
  ...
  output$table <- DT::renderDataTable({
    if(input$databutton == "busStop"){
      ...
      mydata[,c(1,2,3,7,8,9)]
    }else if(input$databutton == "houses"){
      mydata <- as.data.frame(houses_sf)
      mydata[,c(2,5,7,8,9)]
